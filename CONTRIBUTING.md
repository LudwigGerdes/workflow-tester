# Contributing to payload-contract
...
One package: `pnpm --filter payload-contract-engine test`. One file:
...
block added to `USAGE` there — that is what `payload-contract <command> --help` prints.
...
- No network calls outside `payload-contract-instance` and the vendor ingest script.
