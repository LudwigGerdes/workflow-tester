...
![workflow-tester run catching a swallowed expression error](https://raw.githubusercontent.com/LudwigGerdes/workflow-tester/main/docs/demo/quickstart.gif)
...
```bash
npx workflow-tester --help                 # run it without installing
...
```bash
git clone https://github.com/LudwigGerdes/workflow-tester && cd workflow-tester
pnpm install && pnpm build
alias workflow-tester="node $PWD/packages/cli/dist/bin.js"   # every example below assumes this
```
...
```
$ workflow-tester init
created .workflow-tester/tests/example.test.yaml
created .workflow-tester/README.md

No n8n version recorded, so workflow-tester uses the descriptions bundled inside it.
To match your instance: `workflow-tester node-types --instance <url>`, then put
`n8nVersion: <version>` in .workflow-tester/config.yaml.

Point it at a real workflow, then run `workflow-tester run`.
...
- **As an n8n builder, I want** to write a case by hand in a minute **so that** the one payload I know broke last week becomes a permanent test → `.workflow-tester/tests/*.test.yaml`, `workflow-tester schema`
- **As someone using an LLM to write tests, I want** an exact JSON Schema for the test file **so that** the model cannot guess at the format and unknown keys are rejected with a line number → `workflow-tester schema`
...
flowchart LR
  ...
  T[.workflow-tester/tests/*.test.yaml] --> E
  G --> S[.workflow-tester/cases, committed]
...
From `workflow-tester --help`:

| Command | What it does |
|---|---|
| `workflow-tester contracts add <workflow.json> --vendor <v> --events <a,b> [--trigger <name>]` | Write a contract beside the workflow and materialise its shape files. Events are validated before anything is written; a later `--events` replaces the old. |
| `workflow-tester contracts update [<workflow.json>...] [--all] [--fetch] [--vendor <v>]` | Re-materialise from the vendored catalogs. `--fetch` re-downloads the vendor spec first. |
| `workflow-tester gen [<workflow.json>...] [--max N] [--check]` | Generate the variant cases for each contract into `.workflow-tester/cases`. `--check` writes nothing and exits 1 when regeneration would change something. |
| `workflow-tester run [<workflow.json>] [--only generated\|tests] [--format <f>] [--fail-on warn] [--concurrency N]` | Run every case through the tier-1 engine. Formats: stylish, json, junit, sarif, github-actions. |
| `workflow-tester explain <caseId>` | Print a case as a test file, plus what the last run made of it. |
| `workflow-tester schema` | Print the JSON Schema for a test file. |
| `workflow-tester init [--n8n-version <v>] [--no-ask]` | Scaffold `.workflow-tester` with a commented example test. |
| `workflow-tester promote <caseId> [--name <file>]` | Copy a generated case into a hand-written test and keep it from being retired. |
| `workflow-tester capture <workflow.json> --execution <file.json> \| --instance <url> [--workflow <id>] [--awaiting] [--update]` | Record what each node produced, as shape only, into the workflow's sidecar. |
| `workflow-tester sync [--instance <url>] [--interval 30s] [--once]` | Compare every capture in the repo against the instance. |
| `workflow-tester vendors list` / `workflow-tester vendors audit` | Vendors, coverage, spec versions; the per-vendor audit. |
| `workflow-tester node-types --version <v> \| --instance <url> \| --from <dir>` / `--list` | Extract the node descriptions for one n8n release; list what is available. |

`workflow-tester <command> --help` prints one command's usage; `workflow-tester --version` prints the version. Exit codes: `0` clean, `1` findings (or warnings with `--fail-on warn`), `2` usage or configuration error.

### run

Hand-written tests live in `.workflow-tester/tests/*.test.yaml`. A payload is just the body; workflow-tester wraps it in the envelope a Webhook node delivers, so `$json.body.…` resolves the way your expressions are written. Expectations are flat, dotted keys:
...
```
$ workflow-tester contracts add workflows/signup.json --vendor github --events issues,pull_request
...
$ workflow-tester contracts add workflows/signup.json --vendor github --events issues-opened,pull-request-opened
workflows/signup.contract.yaml
  vendor  github @ 1.1.4
  events  issues-opened, pull-request-opened
  shape   ../.workflow-tester/contracts/github.multi-c1e28918.schema.json
```
</details>
...
`gen` turns the contract into the cases worth running and writes them under `.workflow-tester/cases/`:
...
![run --only generated on realistic GitHub cases](https://raw.githubusercontent.com/LudwigGerdes/workflow-tester/main/docs/images/workflow-tester-2.png)
...
note: n8n 2.38.3 is not bundled; using the nearest older 2.10.0
      `workflow-tester node-types --version <yours>` to match your instance
```
</details>
...
`promote <caseId> --name missing-assignee` copies a generated case into `.workflow-tester/tests/` with a `then:` block to fill in and records the id so regeneration stops retiring it.
...
workflow-tester reads n8n's node descriptions to know which parameters are required and when they apply. A trimmed set for n8n 2.10.0 ships inside the package, so a fresh install works offline. To match your instance:

```bash
workflow-tester node-types --version 2.38.3                    # from the npm registry, once
workflow-tester node-types --instance https://n8n.example.com  # asks the instance which version it runs (no credential)
workflow-tester node-types --from ./my-custom-nodes            # custom nodes, offline
workflow-tester init --n8n-version 2.38.3                      # or put n8nVersion: 2.38.3 in .workflow-tester/config.yaml
```

```
$ workflow-tester node-types --list
pinned version: 2.38.3 (.workflow-tester/config.yaml)

bundled:   2.10.0 (inside workflow-tester, always available)
...
Extracted sets live in `~/.workflow-tester/node-types/` (`WORKFLOW_TESTER_CACHE` moves it). `n8nVersion` lives in git because it describes the workflows in the repository, the same for everyone who clones it.
...
.workflow-tester/contracts/    materialised schema + examples (committed)
.workflow-tester/cases/        generated cases, one JSON per case plus index.json (committed)
.workflow-tester/tests/        hand-written and LLM-written tests (committed)
.workflow-tester/reports/      the last run (gitignored)
.workflow-tester/config.yaml   n8nVersion
```

## Editor, CI and AI integration

**Pre-commit (lefthook).** [`lefthook.yml`](lefthook.yml) and [`scripts/workflow-tester.sh`](scripts/workflow-tester.sh) are in this repo. Copy the script into the repo holding your workflows, point `WORKFLOW_TESTER_HOME` at this built checkout, and `npx lefthook install`. (With the npm package installed in that repo, the two hook commands are simply `npx workflow-tester gen --check` and `npx workflow-tester run`.) Every commit then runs `gen --check` and `run`; without the checkout the hook prints one line and exits 0. Details in [`docs/ci.md`](docs/ci.md).

**GitHub Actions.** [`.github/workflows/ci.yml`](.github/workflows/ci.yml) is a working recipe: after `pnpm build`, `node packages/cli/dist/bin.js gen --check` then `run --format sarif > workflow-tester.sarif`, uploaded as an artifact. SARIF regions are file-level because workflow-tester does not read the workflow file's text; `--format github-actions` prints `::error file=…` annotations directly.

**LLMs.** Hand `workflow-tester schema` to a model and let it write cases: the schema is exact (draft 2020-12, with descriptions), unknown keys are rejected with a JSON pointer and line number, and a payload is just the body. There is no MCP server.

## Why not …?

| Alternative | What it gives you | Use that instead when… |
|---|---|---|
| Clicking "Test workflow" in n8n | One execution against one payload, with the real nodes | you need the HTTP calls and credentials to actually run — workflow-tester stops there and says so |
| n8n's own expression validation | Syntax errors in the editor | the expression is malformed; workflow-tester is about well-formed expressions that resolve to `undefined` on a payload you never tried |
| A mock server for the webhook ([integration-mock](https://github.com/LudwigGerdes/integration-mock)) | Replays real requests into a running instance | you want the whole workflow executed end to end; workflow-tester needs no instance and runs in under a second |
| Reviewing the JSON by hand | Judgement | the question is design, not data shape; workflow-tester only knows what a vendor can send |

## FAQ

**Does it change my workflow?** No. It reads the JSON and writes only under `.workflow-tester/` and the `<workflow>.contract.yaml` sidecar.
...
**Which n8n version?** Expressions are evaluated by `n8n-workflow` 2.38.1; node descriptions bundled are 2.10.0 and can be extracted for any release. Pin yours in `.workflow-tester/config.yaml`.

## Known issues

- **Custom (non-vendor) webhooks have no contract path.** `contracts add` requires `--vendor`, and only GitHub and Stripe ship catalogues. For your own payload shape, write cases by hand in `.workflow-tester/tests/`; a `source.kind` for a local schema or example is not built.
- **Event names are workflow-tester's slugs**, and `vendors list` prints counts, not names. The `contracts add` error lists them; there is no `vendors events <vendor>` yet.
...
workflow-tester is maintained by one person alongside other work. Bugs go to GitHub Issues (use the template and include your n8n version and a minimal workflow JSON). Questions go to Discussions. Expect a first response within about a week; nudge the thread if you hear nothing. Feature requests are welcome but not promised.
