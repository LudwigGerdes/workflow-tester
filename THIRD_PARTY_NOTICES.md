...
workflow-tester is released under the MIT License (see `LICENSE`). The material below
...
- **`n8n-workflow@2.38.1`** — a runtime dependency of `workflow-tester-engine`,
  `workflow-tester-contracts` and other workspace packages. It is loaded unmodified
...
trademarks of their respective owners. workflow-tester is not affiliated with or
endorsed by n8n GmbH.

workflow-tester runs without the bundled descriptions: when
