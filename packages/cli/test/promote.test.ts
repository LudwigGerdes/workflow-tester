...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'payload-contract-promote-'));
  out = [];
  err = [];
...
describe('promote', () => {
  it('copies the generated payload into a hand-written test', async () => {
    ...
    const file = join(dir, '.payload-contract/tests/promoted-abc123.test.yaml');
  ...
  it('leaves a `then:` skeleton to fill in', async () => {
    ...
    const text = readFileSync(join(dir, '.payload-contract/tests/promoted-abc123.test.yaml'), 'utf8');
  ...
  it('accepts a name', async () => {
    ...
    expect(existsSync(join(dir, '.payload-contract/tests/missing-email.test.yaml'))).toBe(true);
  });
