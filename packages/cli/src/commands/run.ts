...
import {
  ...
  type ReportFormat,
  type RunReport,
} from 'payload-contract-runner';
import { stringify } from 'yaml';
import { EXIT, parseArgs, type Io } from '../io.js';
import { configPath, readConfig } from '../config.js';

const REPORT_PATH = join('.payload-contract', 'reports', 'last.json');
...
export async function runCommand(argv: string[], io: Io): Promise<number> {
  ...
  if (target !== undefined) {
    ...
    if (!existsSync(file)) {
      io.err(`payload-contract: no such workflow file: ${target}`);
      return EXIT.usage;
    }
    try {
      JSON.parse(await readFile(file, 'utf8'));
    } catch (error) {
      io.err(`payload-contract: ${target} is not a workflow JSON file (${error instanceof Error ? error.message : String(error)})`);
      return EXIT.usage;
    }
  }
  ...
  await mkdir(join(io.cwd, '.payload-contract', 'reports'), { recursive: true });
...
const EXAMPLE = `# A hand-written test. payload-contract runs these exactly like generated cases,
# as far as the expression-pure part of the workflow reaches.
#
# \`payload-contract schema\` prints the JSON Schema for this file — point an LLM at it and
...
#       trigger: webhook
#       # Just the body: payload-contract wraps it in the envelope a Webhook node
...
const README = `# .payload-contract

Everything payload-contract keeps for this repo.
...
which cases changed. \`payload-contract gen --check\` fails when they are stale.
`;
...
export async function initCommand(io: Io, argv: string[] = []): Promise<number> {
  ...
  if (given !== undefined && !/^\d+\.\d+/.test(given)) {
    io.err(`payload-contract: "${given}" is not an n8n version — try 2.38.3`);
    return EXIT.usage;
  }

  const testsDir = join(io.cwd, '.payload-contract', 'tests');
  const example = join(testsDir, 'example.test.yaml');
  const readme = join(io.cwd, '.payload-contract', 'README.md');
  ...
  io.out('created .payload-contract/tests/example.test.yaml');
  io.out('created .payload-contract/README.md');

  // Which n8n release these workflows run on. Skip is a real answer — the
  // descriptions bundled inside payload-contract work — so this never blocks and never
  ...
  if (answer !== undefined && answer !== '' && /^\d+\.\d+/.test(answer)) {
    ...
    io.out(`created .payload-contract/config.yaml (n8n ${answer})`);
  } else {
    io.out('');
    io.out('No n8n version recorded, so payload-contract uses the descriptions bundled inside it.');
    io.out('To match your instance: `payload-contract node-types --instance <url>`, then put');
    io.out('`n8nVersion: <version>` in .payload-contract/config.yaml.');
  }

  io.out('');
  io.out('Point it at a real workflow, then run `payload-contract run`.');
  return EXIT.ok;
}
...
export async function explainCommand(argv: string[], io: Io): Promise<number> {
  const [caseId] = argv;
  if (caseId === undefined) {
    io.err('usage: payload-contract explain <caseId>');
    return EXIT.usage;
  }

  const reportFile = join(io.cwd, REPORT_PATH);
  if (!existsSync(reportFile)) {
    io.err('no report yet — run `payload-contract run` first');
    return EXIT.usage;
  }
  ...
  if (found === undefined) {
    io.err(`no case "${caseId}" in .payload-contract (looked in generated cases and written tests)`);
    return EXIT.usage;
  }
