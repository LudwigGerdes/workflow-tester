...
import { loadSuites } from 'workflow-tester-runner';
...
export async function promoteCommand(argv: string[], io: Io): Promise<number> {
  ...
  if (caseId === undefined) {
    io.err('usage: workflow-tester promote <caseId> [--name <file>]');
    return EXIT.usage;
  }
  ...
  if (found === undefined) {
    io.err(`no generated case "${caseId}" in .workflow-tester/cases — run \`workflow-tester gen\` first, or check the id`);
    return EXIT.usage;
  }
  ...
  const file = [
    `# Promoted from a generated case by \`workflow-tester promote ${caseId}\`.`,
    '#',
    ...
    '#',
    '# `workflow-tester schema` prints the full format.',
    '',
    body,
  ].join('\n');
  ...
  io.out('');
  io.out('Add a `then:` block saying what should happen, then run `workflow-tester run`.');
  return EXIT.ok;
}
