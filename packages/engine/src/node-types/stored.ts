 ...
 *
 * `WORKFLOW_TESTER_CACHE`, deliberately not `WORKFLOW_TESTER_HOME` — that one already names the
 * workflow-tester checkout for `scripts/workflow-tester.sh`, and pointing a cache at a source
 * tree would be a silent, confusing failure.
 */
export function cacheRoot(env: Record<string, string | undefined> = process.env): string {
  return env['WORKFLOW_TESTER_CACHE'] ?? join(homedir(), '.workflow-tester');
}
