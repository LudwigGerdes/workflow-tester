...
package and extraction time. workflow-tester reads them to know which node parameters
...
the descriptions are trademarks of their respective owners. workflow-tester is not
...
proves it. `workflow-tester node-types --version <v>` extracts a full set into
`~/.workflow-tester/node-types/` instead.
