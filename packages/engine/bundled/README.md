...
package and extraction time. payload-contract reads them to know which node parameters
...
the descriptions are trademarks of their respective owners. payload-contract is not
...
proves it. `payload-contract node-types --version <v>` extracts a full set into
`~/.payload-contract/node-types/` instead.
