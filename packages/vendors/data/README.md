...
`<vendor>/<specVersion>/catalog.json` is what workflow-tester knows about a vendor's
...
(`scripts/ingest.ts`), or `workflow-tester contracts update --fetch`. Both download
the upstream spec into `~/.workflow-tester/vendor-specs/` and rewrite the catalog.
