...
[Security tab](https://github.com/LudwigGerdes/payload-contract/security/advisories/new)
...
payload-contract is offline by design. Four commands can reach the network, each only
...
disk. The files payload-contract creates are: `.payload-contract/` in your repository
...
`~/.payload-contract/` (`PAYLOAD_CONTRACT_CACHE` moves them) holding vendor specs and extracted
